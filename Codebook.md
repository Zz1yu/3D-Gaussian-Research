...
2. **梯度回传与特征优化**：
   - 计算损失函数（伪特征损失$\(L_p\)$）后，反向传播时将量化特征 $\(c_j\)$ 的梯度直接复制给对应的原始连续特征$ \(f_i\)$，即 $\(\frac{\partial L_p}{\partial f_i} = \frac{\partial L_p}{\partial c_j}\)$ 
   - 通过梯度下降更新原始连续特征$ \(f_i\)$，使其更接近对应的量化特征 $\(c_j\)$。

3. **码本参数更新**：
   - 基于所有高斯点的索引$ \(I\)$，统计每个量化特征 $\(c_j\) $对应的原始特征集合；
   - 用该集合的特征均值更新量化特征 $\(c_j\)$，即$\(c_j = \frac{1}{N_j} \sum_{i: I_i=j} f_i\)（\(N_j\)$ 为索引为j的高斯点数量）；
